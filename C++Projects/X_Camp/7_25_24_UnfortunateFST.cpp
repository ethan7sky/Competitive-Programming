...
void solve() {
    ...
    for (int rep = 0; rep < k; rep++) {
        ...
        for (int i = 0; i < n; i++) {
            // a[j] + i^2 + j^2 - 2*i*j
            if (i) ndp[i] = min(ndp[i], lines.query(i) + (long long) i * i);
            lines.add(-2 * i, pdp[i] + (long long) i * i);
        }
 
        lines.clear();
        for (int i = n-1; i >= 0; i--) {
            // a[j] + i^2 + j^2 - 2*i*j
            if (i < n-1) ndp[i] = min(ndp[i], lines.query(i) + (long long) i * i);
            lines.add(-2 * i, pdp[i] + (long long) i * i);
        }
